    ...
    def Configure(
        ...
        def FillButtonsList(item):
            ...
            for row in range(len(item[1])):
                buttonsListCtrl.InsertStringItem(row, item[1][row][0])
                buttonsListCtrl.SetStringItem(row, 1, item[1][row][1])
                buttonsListCtrl.SetStringItem(row, 2, item[1][row][2])
        ...
        def Validation():
            ...
            if name:
                if newSuff == "":
                    ...
                    buttonsListCtrl.SetStringItem(self.oldSel, 2, suff[2])
                    if labelText.GetValue() == "":
                        labelText.ChangeValue(suff[1])
                        self.remotes[rem][1][self.oldSel][1] = suff[1]
                        buttonsListCtrl.SetStringItem(self.oldSel, 1, suff[1])
                    addEvent.Enable(True)
                    duplEvent.Enable(True)
                    EnableLeftSide(True)
                    panel.dialog.buttonRow.okButton.Enable(True)
