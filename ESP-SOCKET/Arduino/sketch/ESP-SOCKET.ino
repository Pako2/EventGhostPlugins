...
#include <ESP8266mDNS.h>              // Zero-config Library (Bonjour, Avahi) http://esp-socket.local
...
String defName = "ESP-SOCKET";     // Your client ID
...
void setup()
{
  ...
  Serial.begin(115200);
  Serial.println();
  Serial.println(F("[ INFO ] ESP-SOCKET v0.0.1"));
  ...
  if (configMode)
  {
    ...
    {
      inAPMode = true;
      Serial.println(F("[ INFO ] ESP-SOCKET is running in AP Mode "));
      WiFi.mode(WIFI_AP);
...
void fallbacktoAPMode() {
  ...
  if (APrslt) {
    ...
    // Add Text Editor (http://esp-socket.local/edit) to Web Server. This feature likely will be dropped on final release.
...
void startServer()
{
  ...
  // Start mDNS service so we can connect to http://esp-socket.local (if Bonjour installed on Windows or Avahi on Linux)
