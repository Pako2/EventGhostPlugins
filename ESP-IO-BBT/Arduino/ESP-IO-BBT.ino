...
#include <ESP8266mDNS.h>              // Zero-config Library (Bonjour, Avahi) http://esp-io-bbt.local
...
String bbt_cmdtopic;
String bbt_msgtopic;
const char clientID[] = "ESP8266-IO";     // Your client ID
        ...
        else
        {
          ...
          else if (strcmp(command, "toggle")  == 0)
          {
            ...
            if (root.containsKey("token"))
            {
              ...
              sendAnswer(client, id, res, token); //client missing !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            }
            sendChange(id, pindata[ix].title, res);
            if (psclient.connected())
            {
              publishChange(id, pindata[ix].title, res);
            }
          }
            ...
            sendAnswer(client, id, res, token);
          }
...
void fallbacktoAPMode() {
  ...
  Serial.println(WiFi.softAP("ESP-IO-BBT") ? "Ready" : "Failed!");
  // Access Point IP
  ...
  // Add Text Editor (http://esp-io-bbt.local/edit) to Web Server. This feature likely will be dropped on final release.
...
bool loadConfiguration()
{
  ...
  // Start mDNS service so we can connect to http://esp-io-bbt.local (if Bonjour installed on Windows or Avahi on Linux)
  ...
  {
    inAPMode = true;
    Serial.println(F("[ INFO ] ESP-IO-BBT is running in AP Mode "));
    WiFi.mode(WIFI_AP);
    Serial.print(F("[ INFO ] Configuring access point... "));
    IPAddress apIP;
  ...
  if (data_.success()) {
    if (data_.containsKey("command"))
    {
      ...
      if (command == "getconf")
      {
        DynamicJsonBuffer jsonBuffer;
        JsonObject& data = jsonBuffer.createObject();
        data["message"]  = "configfile";
        data["wled"]     = wifiled;
        JsonArray& pins = data.createNestedArray("gpios");
        for (uint8_t i = 0; i < 10; i++)
        {
          JsonArray& pin_ = pins.createNestedArray();
          pin_.add(pindata[i].id);
          pin_.add(pindata[i].enabled);
          pin_.add(pindata[i].title);
          pin_.add(pindata[i].out);
        }
        publish(data);
      }
      else if (command == "getpinstate")
      {

        Serial.print(F("[ INFO ] Got command: getpinstate("));
        String id = data_["id"].as<String>();
        String token = data_["token"].as<String>();
        Serial.print(id);
        Serial.println(F(")"));
        uint8_t p = pinNum(id);
        if (p == A0) {
      ...
      else if (command == "toggle")
      {
        ...
        ix = pinsS.indexOf(id) / 2;
        oldvals[ix] = rslt;
        if (data_.containsKey("token"))
        {
          String token = data_["token"].as<String>();
          //sendAnswer(id, rslt, token);
          sendMQTTanswer(id, rslt, token);
        }
        sendChange(id, pindata[ix].title, rslt);
        //if (psclient.connected())
        //{
        publishChange(id, pindata[ix].title, rslt);
        //}
      }
      else if (command == "setpinstate")
      {
        Serial.print(F("[ INFO ] Got command: setpinstate("));
        String id = data_["id"].as<String>();
        uint8_t value = data_["value"];
        String token = data_["token"].as<String>();
        Serial.print(id);
        Serial.print(F(", "));
        Serial.print(value);
...
void publishChange(String id, String title, uint16_t val) {
  DynamicJsonBuffer jsonBuffer;
  JsonObject& data = jsonBuffer.createObject();
  data["message"] = "change";
  data["id"] = id;
